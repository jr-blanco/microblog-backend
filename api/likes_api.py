...
@hug.directive()
def jobs(url='127.0.0.1', port=11300, **kwargs):
  return greenstalk.Client((url, port), use='likes')
...
# inserts into likes jobs queue
@hug.put("/likes/{post_id}", status=hug.falcon.HTTP_202)
def add_likes(
  response, 
